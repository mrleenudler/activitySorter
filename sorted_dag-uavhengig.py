...
fulltNavn = df['Skriv inn ditt fulle navn:'].tolist() # unique Identifier
klasse = df['Hvilken klasse går du i?'].tolist() # Class
Aktivitet1 = df['Hvilken aktivitet vil du være med på mandag 10. juni?'].tolist() # Acting as wanted group
...
def structure_applications(inputApplicatiions):
    allApplications = []
    for tup in inputApplicatiions:
        for aktivitet1 in tup[14].split(';'):
            if aktivitet1: # Fixing trailing ';' (perhaps)
                allApplications.append((tup[8], tup[11], aktivitet1, highPriorityString)) # navn == [8], klassse == [11], aktivitet1 == [14], aktivitet2 == 17 
...
fordeling = {"Orientering": [], "Frisbee Ultimate": [], "Kino i hall 2": [], "Langtur i Østmarka": [], "Python nybegynner": [], "Brettspilldag": []}
  ...
  # place_student(allApplications, fordeling, student,     eleverMedBekreftedeAktiviteter, allePaameldteElever)
    ...
    for application in list(allApplications):
        ...
        if application[0] == studentName:
            ...
            print("APPLICATION: ", application[2])
            ...
            allApplications.remove(application)
            #print("Removing application and placing student in place_student()", len(allApplications))
            fordeling[application[2]].append(studentName) # FUNKER
