...
def structure_applications(inputApplications, fordelingDag):
    ...
    for tup in inputApplications:
        ...
        for aktivitet1 in tup[17].split(';'): # 9 er navn 14 er klasse, 17 er aktivitet
            if aktivitet1: # Fixing trailing ';' (perhaps)
                ...
                allApplications.append((tup[9], tup[14], aktivitet1, highPriorityString, aktivitetsdag)) # navn == [9], klassse == [14], aktivitet1 == [17] 
    ...
    #DEBUG / INFO: Hva ønsker de med bare 1-2-3 aktiviteter?
    #              Hva ønsker de som velger Python+ i tillegg??
    # listOfPythonStudents = []
    # for app in allApplications:
    #     if app[2] == "Python for de som kan litt fra før":
    #         listOfPythonStudents.append(app[0])
    # for app in allApplications:
    #     if app[0] in listOfPythonStudents:
    #         print(app[0], "\t", app[2])
...
# Restructuring appliations, creating one application for each desired activity
print("Restructuring allApplications")
allApplications = structure_applications(allApplications, fordelingDag)
# for app in allApplications:
#     print(app)
...
#    print("counter: ", counter)
    ...
    # DEBUG / INFO - lærte ikke så mye
    # for application in allApplications:
    #     if application[0] in groups[1]:
    #         print("Single application: ", application[2])
    #     if application[0] in groups[2]:
    #         print("Two: ", application[2])
...
def place_student(allApplications, fordeling, studentName, eleverMedBekreftedeAktiviteter, allePaameldteElever):
    ...
    for application in list(allApplications):
        if application[0] == studentName:
            ...
            ## Bug! Det er noe feil med dagene
            # Metoden plasserer ikke Python først!

            # Hack to place students applying for "Python for de som kan litt fra før" first 
            if studentName in listOfStudentsWithPythonApplications:
                if application[2] == "Python for de som kan litt fra før":
...
for app in allApplications:
    if app[2] == "Python for de som kan litt fra før":
        listOfStudentsWithPythonApplications.append(app[0])
#         print(app[2], app[0])
#         print("Elevaktiviteter: ", eleverMedBekreftedeAktiviteter[app[0]])
#         allApplications, fordeling, eleverMedBekreftedeAktiviteter, allePaameldteElever = place_student(allApplications, fordeling, app[0], eleverMedBekreftedeAktiviteter, allePaameldteElever)
...
for activity in dictOfActivities:
    dictOfActivities[activity] = sorted(dictOfActivities[activity], key=lambda x: x[0])

for klasse in dictOfClasses:
    dictOfClasses[klasse] = sorted(dictOfClasses[klasse], key=lambda x: x[0])
